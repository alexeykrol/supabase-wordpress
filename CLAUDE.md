...
## 🔍 КРИТИЧНО: Debugging Protocol

**⚠️ ПЕРЕД решением ЛЮБОЙ проблемы:**

1. **ВСЕГДА читай `TROUBLESHOOTING.md` ПЕРВЫМ!**
   - Проверь есть ли проблема уже задокументирована
   - Посмотри что НЕ РАБОТАЛО в прошлом (экономия времени!)
   - Следуй диагностическому workflow

2. **Проверь логи ПЕРЕД предложением решения:**
   ```bash
   # Docker logs для WordPress
...
3. **Определи уровень проблемы:**
   - Frontend (JavaScript console)?
   - Network (Browser DevTools → Network)?
   - Backend (PHP logs)?
   - Database (WordPress DB)?

4. **Root Cause Analysis ДО решения:**
   - Не решай симптомы!
   - Найди ПЕРВУЮ точку отказа
   - Проверь PIDs и timestamps в логах (race condition?)

5. **Документируй решение в TROUBLESHOOTING.md:**
   - Добавь новую проблему
   - Документируй что НЕ работало
   - Добавь команды для диагностики
